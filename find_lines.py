...
def find_lines(img, Line):
    ...
    def find_window_centroids(image, window_width, window_height, margin):
        ...
        # else :
            # l_sum = np.sum(img[int(3 * img.shape[0] / 4):, :int(Line.bestx[0] + 50)], axis=0)
            # l_center = np.argmax(np.convolve(window, l_sum)) - window_width / 2
            # r_sum = np.sum(img[int(3 * img.shape[0] / 4):, int(Line.bestx[1] - 50):], axis=0)
            # r_center = np.argmax(np.convolve(window, r_sum)) - window_width / 2 + int(img.shape[1] / 2)

            # l_center = Line.bestx[0]
            # r_center = Line.bestx[1]

        laneWidth = r_center - l_center
        # print(laneWidth, Line.bestx)
        if (laneWidth < 400 or laneWidth > 700) and len(Line.bestx) > 0:
            l_center = Line.bestx[0]
            r_center = Line.bestx[1]
        ...
        for level in range(1, int(img.shape[0] / window_height)):
            ...
            l_max_index = int(min(l_center + offset, img.shape[1]))
            ...
            # if l_center_new > l_center and l_center_new < (int(img.shape[1] / 2) + 250):
            #     l_center = l_center_new
            # if r_center_new < r_center and r_center_new > (int(img.shape[1] / 2) + 250):
            #     r_center = r_center_new

            window_centroids.append((l_center, r_center))
            # if (r_center - l_center < 300):
    ...
    return output, yaxis, leftx, rightx
